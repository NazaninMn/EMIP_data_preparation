...
def compute_accuracy(pred, gt, radius, return_distance=False, test=False):
    ...
    # TODO: The input data should have 1 channel
    ...
    for i in range(Ng):  # for each gt point, find the nearest pred point
        ...
        if np.min(dist) < radius:  # the nearest pred point is in the radius of the gt point
            ...
            true_pos.append(pred_points[pred_idx])
    ...
    if return_distance and test:
        return TP, FP, FN, d_list, pred_points, true_pos
    else:
        return TP, FP, FN
